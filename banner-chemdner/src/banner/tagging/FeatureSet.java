...
public class FeatureSet implements Serializable {
    ...
    public FeatureSet(TagFormat format, Lemmatiser lemmatiser, dragon.nlp.tool.Tagger posTagger, banner.tagging.Tagger preTagger, String simFindFilename, Set<MentionType> mentionTypes, OverlapOption sameType, OverlapOption differentType, BrownClusters brownClusters, WordEmbeddings wordEmbeddings, WordVectorClasses wordVectorClasses) {
        pipe = createPipe(format, lemmatiser, posTagger, preTagger, simFindFilename, mentionTypes, sameType, differentType, brownClusters, wordEmbeddings, wordVectorClasses);
    }
    ...
    public void setBrownClusters(BrownClusters brownClusters) {
        ((BrownCusterPipe) pipe.getPipe(3)).setBrownClusters(brownClusters);
    }
    
    public void setWordEmbeddings(WordEmbeddings wordEmbeddings) {
        ((WordEmbeddingPipe) pipe.getPipe(4)).setWordEmbeddings(wordEmbeddings);
    }
    
    public void setWordVectorClasses(WordVectorClasses wordVectorClasses) {
        ((WordVectorClassPipe) pipe.getPipe(5)).setWordVectorClasses(wordVectorClasses);
    }
    ...
    private SerialPipes createPipe(TagFormat format, Lemmatiser lemmatiser, dragon.nlp.tool.Tagger posTagger, banner.tagging.Tagger preTagger, String simFindFilename, Set<MentionType> mentionTypes,
            OverlapOption sameType, OverlapOption differentType, BrownClusters brownClusters, WordEmbeddings wordEmbeddings, WordVectorClasses wordVectorClasses) {
        ...
        pipes.add(new BrownCusterPipe(brownClusters));
        pipes.add(new WordEmbeddingPipe(wordEmbeddings));
        pipes.add(new WordVectorClassPipe(wordVectorClasses));
