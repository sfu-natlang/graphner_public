...
import java.util.*; //Golnar added this line
...
public class CRFTagger implements Tagger {
	...
	/**
	 * Loads a {@link CRFTagger} from the specified file. As the lemmatiser and part-of-speech tagger both require data, these cannot be written to disk and must be passed in new.
	 * 
	 * @param f
	 ...
	 */
	public static CRFTagger load(File f, Lemmatiser lemmatiser, dragon.nlp.tool.Tagger posTagger, Tagger preTagger, List<String> cacheFilenames) throws IOException {
		try {
			...
			if (cacheFilenames != null)
				featureSet.setCacheFilenames(cacheFilenames);
	...
	public static CRFTagger load(File f, Lemmatiser lemmatiser, dragon.nlp.tool.Tagger posTagger, Tagger preTagger) throws IOException {
		return load(f, lemmatiser, posTagger, preTagger, null);
	}

	public static CRFTagger train(Set<Sentence> sentences, int order, TagFormat format, FeatureSet featureSet) {
		return train(sentences, order, format, featureSet, 1.0);
	}

	/**
	 * Trains and returns a {@link CRFTagger} on the specified {@link Sentence} s. This method may take hours or even days to complete. When training, you will likely need to increase the amount of
	 * memory used by the Java virtual machine (try adding "-Xms1024m" to the command line).
	 * 
	 * @param sentences
	 ...
	 */
	public static CRFTagger train(Set<Sentence> sentences, int order, TagFormat format, FeatureSet featureSet, double gaussianPriorVariance) {
		if (sentences.size() == 0)
	...
	public void tag(Sentence sentence) {		
		...
		try{
			...
			PrintWriter outLabelsOrder = new PrintWriter(new BufferedWriter(new FileWriter("../../Pipeline/MiddleFiles/LabelsOrder.txt", false))); // Added by Golnar on July 15th, 2016
			...
			Boolean DropLast4Chars = true; // should be true if the features have =1.0 or =0.0 at their end! I have no idea where this came from!
			...
			//Golnar Started April 28th, 2016                       
                        ...
                        LabelAlphabet LA = (LabelAlphabet)instances.getTargetAlphabet();
			//System.out.println("Golnar is saying this: "+LA.lookupLabel(0) + " " + LA.lookupLabel(1) + " " + LA.lookupLabel(2));
                        //Golnar Finished April 28th, 2016
                        SumLatticeDefault sumlattice = new SumLatticeDefault(model, inputSeq, null, null, true, LA);  // LA was added by Golnar on April 28th, 2016
			outLabelsOrder.println(LA.lookupLabel(0) + " " + LA.lookupLabel(1) + " " + LA.lookupLabel(2)); // Added by Golnar on July 15th, 2016

			//Golnar Started July 15th, 2016
			int O_index=-1; 
			int B_index=-1;
			int I_index=-1;
			for (int i=0; i< 3; i++)
			{
				//System.out.println("\""+(LA.lookupLabel(i)).toString()+"\"");	
			...
			//System.out.println(O_index);
			//System.out.println(B_index);
			//System.out.println(I_index);

			//Golnar Finished July 15th, 2016
			...
			{ 				
				...
				for (int j=0; j< featureRepresentation.get(i).size(); j++)
				{
					...
					if (DropLast4Chars)
					{
						...
						//System.out.println("\""+feature+"\"\t\""+Last4Char+"\"");
					}
				...
				for (int s = 0; s < model.numStates(); s++) {
					if (order==2)
					{
						...
						//System.out.println(model.getState(s).getName());
					...
					//System.out.println(model.getState(s).getName()+":"+String.valueOf(sumlattice.getGammaProbability(i+1,s)));
				...
				/*//Golnar started July 15th, 2016
                                out2.print(v.valueAtLocation(O_index)+"\t");
				out2.print(v.valueAtLocation(B_index)+"\t");
				out2.print(v.valueAtLocation(I_index)+"\t");
				//Golnar finished July 15th, 2016*/
			 	...
			 	for (int si = 0; si < model.numStates(); si++)
					for (int sj = 0; sj < model.numStates(); sj++)
					{
						...
						//System.out.println(model.getState(si).getName()+"->"+model.getState(sj).getName()+":"+String.valueOf(twoStateMarginal));
						if (order==1)
				...
				if (order == 2)
				{
					//System.out.println("inside if order==2");
					for (int r=0; r<3; r++)
                        	        {
        	                                double val=0;
	                                        for (int c=0; c<3; c++)
					...
					//System.out.println("exiting if order==2");
			...
			outLabelsOrder.close(); // Added by Golnar on July 15th, 2016
