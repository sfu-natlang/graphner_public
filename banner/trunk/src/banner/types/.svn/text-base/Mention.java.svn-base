...
/**
 * Instances of this class represent the mention of an entity within a {@link Sentence}. Mentions are defined in terms of full tokens, and therefore finding mentions (the job of a {@link Tagger})
 * requires tokenization first.
 * 
 * @author Bob
 */
public class Mention implements Comparable<Mention> {
	...
	public Mention(Sentence sentence, int start, int end, EntityType entityType, MentionType mentionType, Double probability) {
		...
		if (mentionType == null)
			throw new IllegalArgumentException("mentionType cannot be null");
	...
	public Mention copy(Sentence sentence2) {
		Mention mention2 = new Mention(sentence2, start, end, entityType, mentionType, probability);
		mention2.setConceptId(conceptId);
		return mention2;
	}
