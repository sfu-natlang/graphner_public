...
/** Compute F-Score (beta=1) for observations.
Each observation is a triple of longs (long[3]):
- number of true positives
- total number of positives according to model
- total number of positives according to gold standard
**/
...
/**
 * Computation of F-Scores for two datasets and comparison with a randomization test
 * This class extends the (abstract) ApproximateRandomizationTest class. It only 
 * implements main(), readInput(), and calls the FScore class.
 * 
 * @author pado
 *
 */
public class FScoreART extends ApproximateRandomizationTest<Triple<Long>> {
     ...
     */
  public Vector<Triple<Long>> readInput(String filename) throws IOException, FileNotFoundException {
    ...
    while ((line = fileReader.readLine()) != null) {
      ...
      continue;
        ...
        Triple<Long> triple = new Triple<Long>(true_pos, model_pos, gold_pos);
        v.addElement(triple);
    }
    return v;
  }
  ...
  public static void main(String[] args) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
    if (args.length != 3) {
      System.err.println("Usage: FScoreART <file1> <file2> <no. of iterations");
      System.exit(1);
    }    
    ApproximateRandomizationTest<Triple<Long>> art = new FScoreART();
    art.run(args[0], args[1], FScore.class, Long.valueOf(args[2]).longValue()); 
  }
